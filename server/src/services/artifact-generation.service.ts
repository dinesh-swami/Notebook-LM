...
import { findSourcesByWorkspaceId } from "../repository/source.repositry.js";
import type { ArtifactRecord } from "../repository/artifact.repositry.js";
...
export async function gatherSourceContext(
  ...
  const selected = sourceIds?.length
    ? sources.filter((source: { id: string; }) => sourceIds.includes(source.id))
    : sources;
